...
app.get("/purpleAirData/:stationList", async (req, res) => {
  ...
  convertMS = (milliseconds) => {
    ...
    return { day: day, hour: (hour % 24), minute: (minute % 60), seconds: (seconds % 60)};
  };
  ...
  const airQualityFetch = async (staID) => {
    ...
    const {Label, ID, PM2_5Value, Lat, Lon, Flag, Version, RSSI, LastSeen} = airDataJSON.results[0], sta_stats = (airDataJSON.results[0].Stats).split(/[:,"]/);
    const pm25_1hour = Number(sta_stats[15]), pm25_24hour = Number(sta_stats[23]), pm25_1week = Number(sta_stats[27]);
    ...
    const lastSeenUTC = LastSeen + '000', nowTime = new Date();
    const lastSeenTime = new Date(Number(lastSeenUTC));
    const timeSinceSeen = ((nowTime.getTime()) - (lastSeenTime.getTime()));
    const t = convertMS(timeSinceSeen);
    
    airObject = { 'type': 'Feature', 'properties': {'ID':ID,'label':Label,'flagged_high':Flag,'version':Version,'WiFiSignal':RSSI,
                'PM2_5Value':Number(PM2_5Value),'PM2_5_1hour':pm25_1hour,'PM2_5_24hour':pm25_24hour,'PM2_5_1week':pm25_1week,'sensorDiff':diff_val,'AQI':AQI.aqi_val,'AQIText':AQI.aqi,'AQI1Hour':AQI1Hour.aqi_val,'AQI1HourText':AQI1Hour.aqi,
                'AQI24Hour':AQI24Hour.aqi_val,'AQI24HourText':AQI24Hour.aqi,'AQI1Week':AQI1Week.aqi_val,'AQI1WeekText':AQI1Week.aqi, 'lastSeen': lastSeenTime,
                'lastSeenUTC': lastSeenTime.getTime(),'dayHourMinSecSinceSeen': (`${t.day}:${t.hour}:${t.minute}:${t.seconds}`), 'formatSinceSeen': t},
